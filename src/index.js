 ...
 *   /log-design    → Log a new design initiative to Notion
 ...
 * Database: Africhange Design Tracker (Notion)
 */
...
// ═════════════════════════════════════════════════════════════════════════════
// VIEW SUBMIT: log_design_submit — Validate, write to Notion, notify channel
// ═════════════════════════════════════════════════════════════════════════════
app.view('log_design_submit', async ({ ack, body, view, client, logger }) => {
  ...
  try {
    const page = await createEntry(data);
    ...
    // DM the submitting designer with a confirmation + Notion link
    await client.chat.postMessage({
      channel: body.user.id,
      text: `✅ *${data.initiative_name}* has been logged in the Notion tracker.`,
      blocks: [
        {
          type: 'section',
          text: {
            type: 'mrkdwn',
            text: `✅ *${data.initiative_name}* has been added to the tracker.`,
          },
        },
        {
          type: 'actions',
    ...
    logger.error('Error creating Notion entry:', err);
    await client.chat.postMessage({
      channel: body.user.id,
      text: '❌ Something went wrong saving to Notion. Please try `/log-design` again. If it persists, ping your dev team.',
    });
  }
});
...
// ─────────────────────────────────────────────────────────────────────────────
// VIEW SUBMIT: search_to_update — Search Notion, push results select modal
// ─────────────────────────────────────────────────────────────────────────────
app.view('search_to_update', async ({ ack, view, logger }) => {
  ...
  try {
    ...
    logger.error('Error searching Notion for update:', err);
    await ack({
        ...
        blocks: [
          {
            ...
            text: {
              type: 'mrkdwn',
              text: '❌ Could not reach Notion right now. Please try again.',
            },
          },
        ],
      },
...
app.view('select_entry_to_update', async ({ ack, view, logger }) => {
  ...
  try {
    ...
    logger.error('Error fetching Notion entry for edit:', err);
    await ack({
      response_action: 'update',
      view: {
        type: 'modal',
        ...
        blocks: [
          {
            ...
            text: {
              type: 'mrkdwn',
              text: '❌ Could not load that entry from Notion. Please try again.',
            },
          },
        ],
      },
...
// ─────────────────────────────────────────────────────────────────────────────
// VIEW SUBMIT: update_design_submit — Validate, update Notion, notify channel
// ─────────────────────────────────────────────────────────────────────────────
app.view('update_design_submit', async ({ ack, body, view, client, logger }) => {
  ...
  try {
    const page = await updateEntry(pageId, data);
    data.notion_url = page.url;
    ...
    await client.chat.postMessage({
      channel: body.user.id,
      text: `✅ *${data.initiative_name}* has been updated in Notion.`,
      blocks: [
        {
          type: 'section',
          text: {
    ...
    logger.error('Error updating Notion entry:', err);
    await client.chat.postMessage({
      channel: body.user.id,
      text: '❌ Something went wrong updating Notion. Please try `/update-design` again.',
    });
  }
});
...
app.command('/find-design', async ({ command, ack, client, logger }) => {
  ...
  try {
    ...
    logger.error('Error searching Notion for find:', err);
...
// ─────────────────────────────────────────────────────────────────────────────
// VIEW SUBMIT: search_to_find — Search Notion and display results in same modal
// ─────────────────────────────────────────────────────────────────────────────
        ...
        blocks: [
          {
            ...
            text: {
              type: 'mrkdwn',
              text: '❌ Could not reach Notion. Please try again.',
            },
          },
        ],
      },
